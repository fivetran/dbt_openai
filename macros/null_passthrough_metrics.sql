...
{#- Emits a null placeholder for each configured passthrough field, so a union branch that has
    no real value for a passthrough var (e.g. the org-level "other" cost bucket) still lines up
    column-for-column with a sibling branch that does carry real values. -#}
