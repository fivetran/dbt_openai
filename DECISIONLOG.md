...
## Compliance Platform Cost Data Is Staged, Cost-Only
This package also models cost data from Compliance Platform (ChatGPT Enterprise), synced alongside the existing OpenAI Platform data: `stg_openai__compliance_cost`, `stg_openai__compliance_cost_billing`, `stg_openai__compliance_users`, and `int_openai__compliance_cost_rollup`. Only cost data is modeled — no conversation, audit, or auth log data — gated behind `openai_using_compliance_cost` and `openai_using_compliance_users`, both defaulting to `true`.

It's not yet wired into any end-model report, since that requires confirming how it overlaps with the existing `codex_usage`/`codex_usage_model` tables to avoid double-counting.
