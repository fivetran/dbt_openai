...
-- num_model_requests (and any passthrough metric) is one count per source row, not per unit
-- type, so it is only carried on the 'input' branch below — summing it downstream would
-- otherwise triple-count once the row fans out across up to three unit types. The 'input'
-- branch is never filtered by token positivity (unlike 'cache_read'/'output') so a request
-- with zero input tokens still keeps its request count instead of disappearing entirely.
unpivoted as (
