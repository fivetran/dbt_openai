...
with base as (
    ...
    select *
    from {{ ref('stg_openai__audio_speech_tmp') }}
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(ref('stg_openai__audio_speech_tmp')),
                staging_columns=get_audio_speech_columns()
            )
        }}
...
final as (
    ...
    select
        source_relation,
        {{ dbt_utils.generate_surrogate_key(['api_key_id', 'project_id', 'user_id', 'model', 'start_time']) }} as audio_speech_id,
        ...
        {{ dbt.safe_cast('character', dbt.type_int()) }} as characters,
